...
%% expensive. This is the pathological case. If you have an
%% application that relies on an efficient join and it doesn't risk
%% alternating individual out and out_r calls, then this module will
%% be substantially faster than the plain Erlang queue module.
%%
%% len is O(1) as it's tracked explicitly.
...
from_list(L) -> #fq { len = length(L),
                      head = [escape(V) || V <- L], tail = [] }.
